...
public class feedbackController implements Initializable {
    ...
    public void save(MouseEvent event){
        ...
        if(name.isEmpty() || email.isEmpty() || subject.isEmpty() || message.isEmpty()){
            ...
            alert.setContentText("Please fill in completely");
            alert.showAndWait();
        } else{
            qetQuery();
            insert();
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setContentText("Your letter has been sent!");
            alert.showAndWait();
    ...
    private void qetQuery() {
        query = "INSERT INTO car_feedback(name, email, subject, message, message_date) " +
                "VALUES(?,?,?,?,SYSDATE) ";
    }
    private void insert() {
